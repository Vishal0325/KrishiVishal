...
class DeliveryViewModel @Inject constructor(
    ...
    private val _repository: DeliveryRepository,
    ...
    init {
        // REACTIVE SYNC: Observe Auth changes to trigger data loading
        viewModelScope.launch {
            val riderId = currentRiderId
            if (riderId.isNotEmpty()) {
                startDataLoading(riderId)
            }
        }
    }

    private fun startDataLoading(riderId: String) {
        loadIncentives()
        syncData(riderId)
        loadOrders(riderId)
        loadReturns(riderId)
    ...
    fun syncData(riderId: String) = viewModelScope.launch { repository.syncAssignedOrders(riderId) }
    ...
    private fun loadReturns(riderId: String) {
        viewModelScope.launch {
            repository.getAssignedReturns(riderId)
    ...
    fun updateReturnStatus(returnId: String, nextStatus: String) {
        viewModelScope.launch {
            repository.updateReturnStatus(returnId, nextStatus)
        }
    }
    ...
    fun fetchOrderPreview(orderId: String, onError: (String) -> Unit) {
        viewModelScope.launch {
            try {
                val order = repository.fetchOrderForPreview(orderId)
                if (order != null) {
    ...
    fun acceptScannedOrder(orderId: String, onSuccess: (Order) -> Unit, onError: (String) -> Unit) {
        viewModelScope.launch {
            try {
                val order = repository.acceptOrderByScan(orderId, currentRiderId)
                onSuccess(order)
            } catch (e: Exception) {
    ...
    suspend fun verifyDelivery(orderId: String, otp: String): Resource<Unit> {
        val result = repository.verifyOrderDelivery(orderId, otp)
        if (result is Resource.Success) {
            loadOrders(currentRiderId) // Refresh list
        }
        return result
    }

    val repository: DeliveryRepository get() = _repository
}
