...
class GSPProvider {
  ...
  async updateEWayBill(ewbNo, data) {
    throw new Error("Not implemented");
  }
...
class MockGSPProvider extends GSPProvider {
  ...
  async generateEInvoice(order) {
    if (order.totalAmount > 1000000)
      return { status: "FAILED", error: "Limit exceeded" };
    return {
      status: "SUCCESS",
...
async function getGSPProvider() {
  ...
  const gspConfig = config.data()?.gsp || {
    activeProvider: "MOCK",
    mode: "SANDBOX",
  };
  ...
  if (gspConfig.activeProvider === "MOCK" && gspConfig.mode === "PRODUCTION") {
    console.warn(
      "CRITICAL: MOCK GSP Provider requested in PRODUCTION mode. Falling back to error.",
    );
    throw new Error("MOCK provider not allowed in production.");
  }

  if (gspConfig.activeProvider === "CLEARTAX") {
    const authToken = getRequiredSecret(
      "CLEARTAX_AUTH_TOKEN",
      "ClearTax API token",
    );
...
exports.onEmergencyAlertCreated = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    const payload = {
      notification: {
        title: "🚨 EMERGENCY SOS 🚨",
        body: `${data.riderName || "A Rider"} has triggered an SOS!`,
      },
      data: {
...
exports.onUserRoleUpdate = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    // CUSTOM CLAIMS: Single Source of Truth
    // Firebase Auth Custom Claims are the ONLY authorization authority.
    ...
    const normalizedRole =
      ROLE_NORMALIZATION[String(role).trim().toUpperCase()] || role;

    if (!CANONICAL_ROLES.has(normalizedRole)) {
      console.error(
        `Invalid role "${role}" for user ${context.params.userId}. Falling back to Customer.`,
      );
      role = "Customer";
    } else {
      role = normalizedRole;
    ...
    // Legacy admin claim is intentionally NOT created.
...
exports.createOrder = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!cartItems || !Array.isArray(cartItems) || cartItems.length === 0) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Cart must be a non-empty array.",
      );
    }
    ...
    if (cartItems.length > 50) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Cart exceeds maximum allowed items.",
      );
    }
    ...
    if (
      !address ||
      (typeof address !== "object" && typeof address !== "string")
    ) {
      throw new functions.https.HttpsError(
        "invalid-argument",
    ...
    try {
      ...
      const result = await db.runTransaction(async (transaction) => {
        ...
        const costRefs = cartItems.map((item) =>
          db.collection("product_costs").doc(item.productId),
        );
        ...
        costSnaps.forEach((snap) => {
          if (snap.exists) costDataMap[snap.id] = snap.data();
        });
        ...
        for (const item of cartItems) {
          ...
          if (!Number.isInteger(qty) || qty <= 0) {
            throw new Error(
              `Invalid quantity for ${product.name}. Must be a positive integer.`,
            );
          }
          ...
          if (qty > MAX_QTY_PER_ITEM) {
            throw new Error(
              `Quantity for ${product.name} exceeds maximum limit of ${MAX_QTY_PER_ITEM}.`,
            );
          }
          ...
          if (
            item.variantId &&
            costData.variantsCost &&
            costData.variantsCost[item.variantId]
          ) {
          ...
          if (item.variantId) {
            ...
            if (variantStock < qty)
              throw new Error(`Variant for ${product.name} out of stock.`);
...
exports.verifyPayment = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (
      ...
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Missing mandatory payment verification fields.",
      );
    }

    const razorpaySecret = getRequiredSecret(
      "RAZORPAY_KEY_SECRET",
      "Razorpay signing secret",
    );
    const razorpayKeyId = getRequiredSecret(
      "RAZORPAY_KEY_ID",
    ...
    try {
      ...
      const payment = await razorpayBreaker.execute(() =>
      ...
      if (payment.amount !== expectedPaise) {
        console.error(
          `Amount mismatch: Razorpay=${payment.amount}, Order=${expectedPaise}`,
        );
 ...
 */
exports.initiateRefund = functions
  .runWith({ secrets: ["RAZORPAY_KEY_ID", "RAZORPAY_KEY_SECRET"] })
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    ...
    if (!returnId || refundAmount === undefined) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Return ID and refund amount are required.",
      );
    }
    ...
    const keySecret = getRequiredSecret(
      "RAZORPAY_KEY_SECRET",
      "Razorpay signing secret",
    );
    ...
    try {
      const result = await db.runTransaction(async (transaction) => {
        ...
        if (amount > maxRefund) {
          throw new Error(
            `Refund amount ₹${amount} exceeds order total ₹${maxRefund}`,
          );
        }
      ...
      if (result.type === "ONLINE") {
        ...
        const refund = await razorpayBreaker.execute(() =>
          razorpay.payments.refund(result.razorpayPaymentId, {
...
exports.getAiSystemHealth = functions.https.onCall(async (data, context) => {
  ...
  return {
    ...
    pendingApprovals: actions.docs.filter((d) => d.data().status === "PENDING")
      .length,
    completedActions: actions.docs.filter(
      (d) => d.data().status === "COMPLETED",
    ).length,
    rejectedActions: actions.docs.filter((d) => d.data().status === "REJECTED")
      .length,
...
exports.aiSupervisor = functions.https.onCall(async (data, context) => {
  ...
  if (config.data()?.disableAiSupervisor) {
    throw new functions.https.HttpsError(
      "unavailable",
      "AI Supervisor is temporarily disabled by SuperAdmin.",
    );
  }
  ...
  // SLA / Operations
  if (
    ["order", "late", "status", "rider", "delivery", "sla"].some((k) =>
  ...
  // Analytics / Health
  if (
    ["health", "error", "logs", "traffic", "performance", "realtime"].some(
      (k) => lowerPrompt.includes(k),
    )
  )
    agentType = "ANALYTICS";

  let response = {
    message: "I am KrishiVishal AI. How can I help you manage the business?",
    agentType,
  };

  try {
    if (agentType === "ANALYTICS") {
      const tenMinsAgo = admin.firestore.Timestamp.fromMillis(
        Date.now() - 600000,
      );
      ...
      response = {
        ...
        data: {
          errorRate: errorLogs.size / (totalCalls.data().count || 1),
          recentErrors: errorLogs.size,
        },
        agentType: "ANALYTICS",
        verified: true,
      };
      ...
      const accData = accounts.docs.map((d) => ({
        id: d.id,
      ...
      if (lowerPrompt.includes("total") || lowerPrompt.includes("kitne")) {
        ...
        const lowStock = await db
          ...
          .limit(10)
          .get();
        const items = lowStock.docs.map((d) => ({
          id: d.id,
          name: d.data().name,
          stock: d.data().stockQuantity,
          price: d.data().price,
        }));
 ...
 */
exports.getFinanceSummary = functions
  .runWith({
    memory: "512MB",
  })
  .https.onCall(async (data, context) => {
...
exports.processOutbox = functions.firestore
  ...
  .onCreate(async (snap) => {
    ...
    if (event.type === "ORDER_CREATED") {
      await admin
        .messaging()
        .sendToTopic("admin_alerts", {
          notification: {
            title: "New Order",
            body: `Order ${event.payload.orderId} placed.`,
          },
        });
    }
...
exports.onOrderPaidLedger = functions.firestore
  .document("orders/{id}")
  .onUpdate(async (change, context) => {
    ...
    const isNowPaid =
      newData.paymentStatus === "PAID" && oldData.paymentStatus !== "PAID";
    ...
    if (isNowPaid && !newData.ledgerPosted) {
      ...
      if (!existingEntries.empty) {
        console.log(
          `Ledger entry already exists for Order: ${orderId}. Marking as posted.`,
        );
        await db
          .collection("orders")
          .doc(orderId)
          .update({ ledgerPosted: true });
        return null;
      }
      ...
      try {
        ...
        else if (newData.paymentMethod === "ONLINE")
          assetAccount = "RAZORPAY_PENDING";
        ...
        for (const item of items) {
          ...
          batch.set(
            db.collection("sales_stats").doc(statId),
            {
              productId,
        ...
        if (userId) {
          ...
          if (categories.length > 0) {
            batch.update(db.collection("users").doc(userId), {
              interestedCategories: admin.firestore.FieldValue.arrayUnion(
                ...categories,
              ),
            });
          }
 ...
 */
exports.monitorOrderSLA = functions.pubsub
          ...
          await admin
            ...
            .send({
              notification: {
                ...
                body: `Order #${order.id.slice(0, 8)} is waiting for pickup!`,
              },
              token: (
                await db.collection("users").doc(data.riderId).get()
              ).data()?.fcmToken,
            })
            .catch(() => {});
        }
      ...
      if (delayedDeliveries.size > 0) {
        await admin.messaging().sendToTopic("admin_alerts", {
          notification: {
            ...
            body: `${delayedDeliveries.size} orders are taking longer than 1 hour to deliver.`,
          },
        });
      }

      console.log(
        `SLA Audit complete. Nudges sent for ${delayedPickups.size} orders.`,
      );
    } catch (e) {
 ...
 */
exports.onOrderDeliveryUpdate = functions.firestore
  .document("orders/{id}")
  .onUpdate(async (change, context) => {
    const newData = change.after.data();
    const oldData = change.before.data();
    ...
    if (
      finalStates.includes(newData.status) &&
      ...
      await db.runTransaction(async (transaction) => {
        ...
        if (newData.status === "DELIVERED") {
          ...
          if (newData.assignedAt && newData.deliveredAt) {
            const duration =
              newData.deliveredAt.toDate() - newData.assignedAt.toDate();
...
exports.onReturnStatusUpdate = functions.firestore
  .document("returns/{id}")
  .onUpdate(async (change) => {
    const newData = change.after.data();
    if (newData.qcStatus === "PASSED") {
      await db
        ...
        .update({
          stockQuantity: admin.firestore.FieldValue.increment(newData.quantity),
        });
    }
  });

exports.onReturnCompletedLedger = functions.firestore
  .document("returns/{id}")
  .onUpdate(async (change, context) => {
    const newData = change.after.data();
    const oldData = change.before.data();
    ...
    if (
      ...
      if (!existingEntries.empty) {
        console.log(
          `Ledger entry already exists for Return: ${returnId}. Marking as posted.`,
        );
        await db
          .collection("returns")
          .doc(returnId)
      ...
      try {
        ...
        if (orderData && orderData.items) {
          ...
          const returnedItem = orderData.items.find(
        ...
        else if (newData.refundMethod === "ONLINE")
          assetAccount = "RAZORPAY_PENDING";
        ...
        batch.update(db.collection("returns").doc(returnId), {
          ledgerPosted: true,
        });
      ...
      if (fcmToken) {
        await admin
          ...
          .send({
            notification: {
              title: "Order Update",
              body: `Order status is now ${newData.status}`,
            },
            token: fcmToken,
          });
      }
...
exports.sendBroadcastNotification = functions.firestore
  .document("broadcast_notifications/{id}")
  .onCreate(async (snap) => {
...
exports.processAiAction = functions.firestore
  .document("ai_action_requests/{id}")
  .onUpdate(async (change, context) => {
    const newData = change.after.data();
    const oldData = change.before.data();
...
exports.generateEInvoice = functions.https.onCall(async (data, context) => {
  ...
  if (provider instanceof MockGSPProvider && gspConfig.mode === "PRODUCTION") {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Real GSP integration required for production.",
    );
  }
  ...
  try {
    const response = await gspBreaker.execute(() =>
      provider.generateEInvoice(order),
    );
...
exports.recordBankPayout = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    return {
      success: true,
      message: `Payout [${refId}] reconciled successfully.`,
    };
  } catch (error) {
    throw new functions.https.HttpsError("internal", error.message);
  }
...
exports.recordExpensePayment = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    await logAudit({
      action:
        type === "RIDER_PAYOUT" ? "RIDER_PAYOUT_PROCESSED" : "EXPENSE_PAID",
      actorId: context.auth.uid,
      targetId,
      targetType: type,
 ...
 */
exports.deleteExpenseAttachment = functions.https.onCall(
  async (data, context) => {
    requireSuperAdmin(context);
    ...
    try {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        if (!attachment) {
          console.warn(
            `Attachment ${attachmentId} already deleted or not found.`,
          );
          return;
        }
...
exports.generateEWayBill = functions.https.onCall(async (data, context) => {
  ...
  try {
    const response = await gspBreaker.execute(() =>
      provider.generateEWayBill(order),
    );
  ...
  try {
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!amount || isNaN(amount) || amount <= 0)
        throw new Error("Invalid order amount on server.");
      ...
      if (walletBalance < amount) {
        throw new Error(
          `Insufficient wallet balance. Needed: ₹${amount}, Available: ₹${walletBalance}`,
        );
      }
      ...
      return {
        success: true,
        newBalance: Math.round((walletBalance - amount) * 100) / 100,
      };
    });
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (order.paymentStatus === "PAID") {
        ...
        const assetAccount =
          order.paymentMethod === "WALLET" ? "WALLET_BALANCE" : "CASH_IN_HAND";
        transaction.set(db.collection("ledger").doc(), {
          account: assetAccount,
          type: "CREDIT",
          amount: totalAmount,
...
exports.verifyDeliveryOTP = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      const now = Date.now();
      const otpCreatedAt = order.otpCreatedAt
        ? order.otpCreatedAt.toMillis()
        : order.updatedAt
