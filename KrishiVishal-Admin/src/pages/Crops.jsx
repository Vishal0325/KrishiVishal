...
import { Plus, Edit2, Trash2, X, Sprout, Image as ImageIcon, Upload, Link, Loader2 } from 'lucide-react';
...
const Crops = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCrop) {
        await setDoc(doc(db, 'crops', editingCrop.id), data, { merge: true });
  ...
  const columns = [
    ...
    { header: 'Name', key: 'name', render: (c) => <span className="font-black text-gray-900 tracking-tight">{c.name}</span> },
    { header: 'Status', key: 'isActive', render: (c) => (
      <span className={`text-[10px] font-black px-2 py-1 rounded uppercase tracking-widest ${c.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
    ...
    { header: 'Actions', render: (c) => (
      <div className="flex space-x-2">
        ...
        <button onClick={() => deleteCrop(c.id)} className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-all"><Trash2 size={16} /></button>
      </div>
    )}
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl font-black text-gray-900 tracking-tight flex items-center">
          <Sprout className="mr-3 text-[#1b5e20]" size={28} />
          Crop Management
        </h1>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <DataTable columns={columns} data={crops} loading={loading} />
        </div>

        <div className="space-y-6">
          <div className="bg-[#1b5e20] p-8 rounded-3xl text-white shadow-xl shadow-green-100 relative overflow-hidden group">
            <h3 className="text-lg font-black uppercase tracking-widest mb-2 relative z-10">Crops Section</h3>
            <p className="text-sm font-medium text-green-100 leading-relaxed relative z-10">
              Manage the crops shown in the app. Users can select these crops to find related products.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-md p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl animate-in zoom-in duration-300 relative border border-white/20">
            <div className="p-6 border-b border-gray-50 flex items-center justify-between sticky top-0 bg-white z-10">
              <h2 className="text-lg font-black text-gray-900 tracking-tight uppercase tracking-[0.1em]">{editingCrop ? 'Edit Crop' : 'Add Crop'}</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Crop Image</label>
                    <button
                      type="button"
                    ...
                    <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl p-4 bg-gray-50 hover:bg-gray-100 transition-colors relative group">
                      {formData.imageUrl ? (
                        ...
                        <label className="flex flex-col items-center justify-center cursor-pointer w-full py-4">
                          ...
                          <span className="text-xs font-bold text-gray-500">{uploading ? 'Uploading...' : 'Click to upload image'}</span>
                          <input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} disabled={uploading} />
                        </label>
                      )}
                    </div>
